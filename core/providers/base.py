...
- list_models() -> [models]
- get_context_length(model_id) -> int
...
class BaseProvider(ABC):
    ...
    def chat(self, messages: List[Dict], temperature: float = 0.7) -> Tuple[str, Dict[str, Any]]:
        ...
        Returns:
            Tuple de (content: str, usage: dict con prompt_tokens, completion_tokens, total_tokens, duration, tps)
        """
        pass
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            Lista de modelos [{"id": "modelo-1", "context_length": 32768}, ...]
        """
        pass
...
class ProviderFactory:
    ...
    def detect(cls, base_url: str = None) -> str:
        ...
        Prueba en orden: LM Studio -> Ollama -> OpenAI -> Gemini
        """
        import requests
        
        # LM Studio (puerto default 1234)
        if base_url:
            url = base_url
        else:
    ...
    def auto_create(cls, base_url: str = None, **kwargs):
        ...
        Args:
            base_url: URLbase (opcional)
            **kwargs: Argumentos para el provider
        
        Returns:
            Instancia del proveedor detectado
        """
        # Intentar detectar automáticamente
        detected = cls.detect(base_url)
        
        if detected:
            # Determinar proveedor por puerto/URL
        ...
        # Fallback: tentar LM Studio por defecto
